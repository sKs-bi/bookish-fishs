...
const Layout = () => {
  ...
  const menuItems = [
    ...
    { path: '/assets', label: '资产管理', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', children: [
    ...
    { path: '/role-upgrade-approval', label: '审批中心', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4', children: user?.role === 'super_admin' || user?.role === 'dept_admin' ? [
        ...
        <div className="px-2 sm:px-4 lg:px-6">
          <div className="flex justify-between h-14 sm:h-16">
            <div className="flex items-center">
              ...
              <div className="ml-2 sm:ml-3 hidden sm:block">
                ...
                <div className="text-[10px] tracking-[0.15em] text-gray-900 font-medium">
                  DIGITAL <span style={{ color: '#60A5FA' }}>I</span>NTELLIGENCE <span style={{ color: '#FB923C' }}>T</span><span style={{ color: '#111827' }}>ECHNOLOGY</span>
                </div>
              </div>
              <button
                onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                className="ml-2 p-2 rounded-md text-gray-400 hover:text-gray-600 sm:hidden"
              >
            ...
            <div className="flex-1 flex items-center justify-center">
              <span className="text-2xl sm:text-3xl font-bold text-gray-900" style={{ fontFamily: 'KaiTi, STKaiti, serif' }}>数智资产管理</span>
            </div>
        ...
        `}>
          ...
          <nav className="mt-0 lg:mt-5 px-2 py-4 lg:py-5 space-y-0.5 sm:space-y-1 overflow-y-auto max-h-[calc(100vh-3.5rem)] lg:max-h-[calc(100vh-8rem)]">
            {menuItems.map((item) => {
              ...
              const isExpanded = expandedMenus[item.path] !== undefined ? expandedMenus[item.path] : isChildActive;
              return (
                <div key={item.path}>
                  <button
                    onClick={() => {
                    ...
                    className={`${(location.pathname === item.path || isChildActive) ? 'bg-primary text-white' : 'text-gray-600 hover:bg-gray-100'} group flex items-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-medium rounded-lg transition-colors w-full`}
                  >
                    <svg className={`${(location.pathname === item.path || isChildActive) ? 'text-white' : 'text-gray-400'} mr-2 sm:mr-3 h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="ml-4 sm:ml-6 mt-0.5 space-y-0.5">
                      {item.children.map((child) => (
                        <button
                          ...
                          className={`${location.pathname === child.path ? 'bg-primary text-white' : 'text-gray-600 hover:bg-gray-100'} group flex items-center px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg transition-colors w-full`}
                        >
                          <svg className={`${location.pathname === child.path ? 'text-white' : 'text-gray-400'} mr-2 sm:mr-3 h-3.5 w-3.5 sm:h-4 sm:w-4 flex-shrink-0`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
